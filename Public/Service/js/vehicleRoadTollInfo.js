...
(function ($, window) {
    var vehicleRoadTollInfo = {
        ...
        init: function () {
            var that = this;
            $("#grid").datagrid({
                url: '/CourtGms/index.php?m=Service&c=VehicleRoadTollInfo&a=getVehicleRoadTollInfoToList',
                toolbar: '#div_toolbar',
                nowrap: false,
                remoteSort: false,
                border: false,
        ...
        edit: function (value, id) {
            ...
            if (value) {
                $.ajax({
                    url: '/CourtGms/index.php?m=Service&c=vehicleRoadTollInfo&a=' + that.operateFlag,
                    type: 'get',
                    dataType: 'Json',
                    data: {
                        vehicle_id: value,
                ...
                $.messager.confirm('注意', '此操作不可恢复是否确定？', function (r) {
                    if (r) {
                        $.messager.confirm(that.lang.prompt, that.lang.sureDelete, function (r) {
                            if (r) {
                                $.ajax({
                                    url: '/CourtGms/index.php?m=Service&c=vehicleRoadTollInfo&a=Delete',
                                    type: 'Post',
                                    dataType: 'Json',
                                    data: {
                                        vehicle_id: value,
        ...
        exportData: function () {
            ...
            $.ajax({
                url: "/CourtGms/index.php?m=Service&c=VehicleRoadTollInfo&a=exportData",
                type: 'post',
                dataType: 'json',
                data: data,
        ...
        operate: function (value, row, index) {
            // console.log(row);
            // return "<a href='javascript:;' onclick='window.vehicleRoadTollInfo.edit(\"" + row.vehicle_id + "\",\"" + row.damage_time + "\")'>" + window.vehicleRoadTollInfo.lang.edit + "</a>&nbsp;|&nbsp;<a href='javascript:;' onclick='window.vehicleRoadTollInfo.deleteData(\"" + row.vehicle_id + "\",\"" + row.damage_time + "\")'>" + window.vehicleRoadTollInfo.lang.delete1 + "</a>";
            var html = "<div style='height: 30px;padding-top: 10px'>";
            if (result_edit) {
                html = html + "<a href='javascript:;' style='text-decoration: none;' onclick='window.vehicleRoadTollInfo.edit(\"" + row.vehicle_id + "\",\"" + row.id + "\")'>" +
                    "<img style='border:none;width: 36px;' src='/CourtGms/Public/Static/Easyui/themes/icons/edit_add2.png'></a>&nbsp;"
            }
            if (result_del) {
                html = html + "<a href='javascript:;' style='text-decoration: none;' onclick='window.vehicleRoadTollInfo.deleteData(\"" + row.vehicle_id + "\",\"" + row.id + "\")'>" +
        ...
        operateSure: function () {
            ...
            var that = this;
            $("#form_vehicle_Road").form('submit', {
                url: "/CourtGms/index.php?m=Service&c=vehicleRoadTollInfo&a=" + action,
                onSubmit: function () {
        ...
        fillForm: function (obj) {
            ...
            if (obj) {
                ...
                $("#inp_toll_gate_name").textbox('setValue', obj.toll_gate_name);
    ...
    $(function () {
        vehicleRoadTollInfo.init();
        if (window.parent.guide) {
            window.parent.guide.removeLoading();
        }
    });
    return window.vehicleRoadTollInfo = window.vehicleRoadTollInfo || vehicleRoadTollInfo;
})($, window);